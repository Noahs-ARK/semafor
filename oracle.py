...
def calc_oracle(directory, num_ranks):
    ...
    print "Oracle\tMacro \tPrecision  \tRecall \tF1-Score"
	...
	print "%d\t%.5f\t%.5f \t%.5f\t%.5f" % (rank, avg, p, r, microavg)
        if rank == 1:
            ...
            rank += 25
    avg, p, r, microavg = do_micro_avg(fscores, rec_denom, rec_num, pre_denom, pre_num, num_ranks)
    print "%d\t%.5f\t%.5f \t%.5f\t%.5f" % (num_ranks, avg, p, r, microavg)
    print
    return microavg
