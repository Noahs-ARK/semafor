...
class CandidateSpanPrunerTest extends FlatSpec with Matchers {
  ...
  private val prunerWithPunct = CandidateSpanPruner.baseline.copy(doStripPunctuation = false)
  private val prunerWithoutPunct = CandidateSpanPruner.baseline.copy(doStripPunctuation = true)
  ...
  it should "strip DT and target from noun targets" in {
    ...
    val result = CandidateSpanPruner.baseline.copy(doFindNNModifiers = true).candidateSpans(inputSentence, target)
